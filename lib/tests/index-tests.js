...
describe('The entry point', function () {
	...
	describe('the validate method', function () {
		...
		it('should start the local app server if specified in the config', function () {
			...
			mockLocalAppServer.expects('start')
				.once()
				.returns(Promise.resolve());

			/* should be a stub, but reusing mock
			 * ref for the sake of simplicity */
			mockEntryPoint.expects('_isInvalid')
				.returns(isInvalid);

			mockEntryPoint.expects('_cleanup')
				.returns(undefined);

			sinon.stub(resultsPrinter, 'printResults')
				.returns(Promise.resolve());

			sinon.stub(runner, 'run')
				.returns(Promise.resolve());

			return entryPoint.validate(config, isCliRun)
				.then(() => {
					mockLocalAppServer.verify();
				});
		});
